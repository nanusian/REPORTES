#!/usr/bin/env python3
"""
Google Ads - Reporte mensual MEDICAL HAIR
Extrae datos por campaña, genera JSON + HTML con comparación mes anterior.

Uso:
    python3 google_medical_hair_reporte.py 2026-03-01 2026-03-31
"""
...
def generate_html(report_new, report_old, date_from):
    ...
    ctr_new = (resumen_new["clicks_totales"] / resumen_new["impresiones_totales"] * 100) if resumen_new["impresiones_totales"] > 0 else 0
    ctr_old = (resumen_old["clicks_totales"] / resumen_old["impresiones_totales"] * 100) if (resumen_old and resumen_old["impresiones_totales"] > 0) else 0
    ...
    summary_html = f"""
        ...
        <div class="summary-card conversions">
            <div class="value">{format_number(resumen_new["impresiones_totales"])}</div>
            <div class="label">Impresiones</div>
            {change_badge(resumen_old["impresiones_totales"] if resumen_old else 0, resumen_new["impresiones_totales"])}
        </div>
        ...
        <div class="summary-card">
            <div class="value">{ctr_new:.2f}%</div>
            <div class="label">CTR</div>
            {change_badge(ctr_old, ctr_new)}
        </div>
    """
    ...
    for camp_name, c in campanas_new.items():
        ...
        <div class="campaign-card">
            ...
            <div class="metrics-row">
                ...
                <div class="metric-box">
                    <div class="value" style="color: #34A853;">{format_number(c["clicks"])}</div>
                ...
                <div class="metric-box">
                    <div class="value">{format_currency(c["avg_cpm"])}</div>
    ...
    for camp_name, c in campanas_new.items():
        c_old = campanas_old.get(camp_name, {})
        type_label = CAMPAIGN_TYPE_MAP.get(c["campaign_type"], c["campaign_type"])
        clicks_old = c_old.get("clicks", 0)
        cpc_old = c_old.get("avg_cpc", 0)
        ctr_c_old = c_old.get("ctr", 0)

        clicks_change, clicks_dir = calculate_change(clicks_old, c["clicks"])
        ...
        table_rows += f"""
                <tr>
                    ...
                    <td>{type_label}</td>
                    <td>{format_number(clicks_old)}</td>
                    <td>{format_number(c["clicks"])}</td>
                    <td class="change-{clicks_dir}">{clicks_change}</td>
    ...
    sorted_camps = sorted(campanas_new.values(), key=lambda x: x["clicks"], reverse=True)
    insights_html = ""
    ...
    if sorted_camps:
        ...
        <div class="insight-box">
            <h4>✅ Campaña con mayor volumen de clicks</h4>
            <p><strong>{best["campaign_name"]}</strong> generó {format_number(best["clicks"])} clicks
            con un CPC de {format_currency(best["avg_cpc"])} y CTR de {best["ctr"]:.2f}%.</p>
        </div>
        """

    # Insight de CPC
    avg_cpc_global = resumen_new["inversion_total"] / resumen_new["clicks_totales"] if resumen_new["clicks_totales"] > 0 else 0
    for c in campanas_new.values():
        if avg_cpc_global > 0 and c["avg_cpc"] > avg_cpc_global * 1.3:
            insights_html += f"""
        <div class="insight-box warning">
            <h4>⚠️ CPC elevado en {c["campaign_name"]}</h4>
            <p>El CPC de {format_currency(c["avg_cpc"])} está por encima del promedio general ({format_currency(avg_cpc_global)}).
            Revisar palabras clave y segmentación para reducir el costo por click.</p>
        </div>
        """

    if resumen_old and resumen_old["clicks_totales"] > 0:
        clicks_change_pct = ((resumen_new["clicks_totales"] - resumen_old["clicks_totales"]) / resumen_old["clicks_totales"]) * 100
        direction = "mejoró" if clicks_change_pct >= 0 else "bajó"
        insights_html += f"""
        <div class="insight-box info">
            <h4>📊 Evolución mensual</h4>
            <p>El total de clicks {direction} un <strong>{abs(clicks_change_pct):.1f}%</strong> respecto a {mes_anterior_abrev}.
        ...
        <table>
            <thead>
                <tr>
                    <th>Campaña</th>
                    <th>Tipo</th>
                    <th>Clicks {mes_anterior_abrev}</th>
                    <th>Clicks {month_display[month][:3]}</th>
                    <th>Var Clicks</th>
...
def main():
    if len(sys.argv) < 3:
        print("Uso: python3 google_medical_hair_reporte.py FECHA_INICIO FECHA_FIN")
        print("Ej:  python3 google_medical_hair_reporte.py 2026-03-01 2026-03-31")
        sys.exit(1)
